if (window.location.href.search("quizizz.com/join/game/") == -1 && window.location.href.search("gameType=") == -1) {
    if (window.location.href.search("quizizz.com/join/pre-game/") != -1) {
        alert("You cannot execute this while paused. If you are not paused please chat WhatsApp 085156303120!")
        throw new Error("You cannot execute this while paused. If you think this is an error please chat WhatsApp 085156303120!");
    } else if (window.location.href.search("quizizz.com/join/quiz/") != -1) {
        alert("You need to start the game before running this script. If you think this is an error please chat WhatsApp 085156303120!")
        throw new Error("You need to start the game before running this script. If you think this is an error please chat WhatsApp 085156303120");
    } else {
        alert("You aren't on a quizizz quiz. If you think this is an error pleasechat WhatsApp 085156303120!")
        throw new Error("You aren't on a quizizz quiz. If you think this is an error please chat WhatsApp 085156303120!");
    }
}
...
function GetQuestion(Set) {
    for (let v of Object.keys(Set.questions)) {
        ...
        switch (GetQuestionType()) {
            case "Both":
                console.log('Question type: both');
                let BothSRC = document.getElementsByClassName("question-media")[0].children[0].src //question-media audio
                ...
                break
            case "Media":
                console.log('Question type: Media');
...
function QuestionChangedLoop() 
{
    ...
    if (NewNum) 
    {
        console.log('Regular question found');

        if (NewNum.innerHTML != CurrentQuestionNum) 
        {
            console.log('Idfk but it passed');
            setTimeout(function ()
            {
                if (document.getElementsByClassName("typed-option-input")[0]) //Text input
                {
